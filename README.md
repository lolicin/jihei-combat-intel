...
| **接管瞄准**（写 `MouseTarget`） | ❌ **未生效** | 托管 `SystemBase` 在该世界不被驱动，见 [`mod/README.md` §7.4](mod/README.md) |
| 接管移动 / 技能 | ⛔ 未开始 | 依赖上一条打通 |

**已知失败是诚实记录的**，不是没做完就删掉——`AimOverrideSystem.cs` 连同排错结论一起留着，
因为"注册成功、日志漂亮、实际什么都不干"这个坑本身值得记下来。
...
.\install.ps1        # 装加载器 + 插件
# 启动游戏，进局内：F8 建议层 / F9 浮层 / F10 落盘 / F11 雷达 / F12 血条
```
